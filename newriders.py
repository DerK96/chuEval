...
def count_unique_names(table0_html, table1_html, table2_html, table3_html):
    ...
    count = 0
    for name in names_table3:
        if name not in names_table2 and name not in names_table1 and name not in names_table0:
            count += 1
...
if __name__ == '__main__':
    ...
    with open('RiderRankingListCombined_24.html', 'r') as file3:
        html_content3 = file3.read()

    # Count unique names appearing only in the second table
    unique_names_count = count_unique_names(html_content0, html_content1, html_content2, html_content3)

    print("Number of persons appearing only in the third table:", unique_names_count)
